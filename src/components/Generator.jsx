          ...
          <div className='flex flex-col px-3 pb-3'>
            {(poison === 'individual' ? WORKOUTS[poison] : Object.keys(WORKOUTS[poison])).map((muscleGroup,muscleGroupIndex)=>{
              return(
                ...
                }} key={muscleGroupIndex} className={'hover:text-blue-400 duration-200 cursor-pointer ' + (muscles.includes(muscleGroup)? 'text-blue-400' : ' ')}>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-3 gap-4'>
        {Object.keys(SCHEMES).map((scheme,schemeIndex) => {
            return(
              <button onClick={()=>{
                setGoal(scheme)
              }} className={'bg-slate-950 border duration-200 hover:border-blue-600 py-3 rounded-lg px-4 cursor-pointer '+ 
